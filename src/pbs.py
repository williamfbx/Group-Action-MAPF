...
def creates_cycle(priority_pairs):
    tg = TopologyGraph(directed=True)
    tg.clear_graph()
    for u, v in priority_pairs:
        tg.Edge(u, v)
    return tg.is_cyclic()
...
class PBSSolver(object):
    ...
    def update_plan(self, node, i):
        ...
        to_update = get_lower_priority_agents(node['priority_pairs'], i)
            
        for agent in to_update:
            higher = get_higher_priority_agents(node['priority_pairs'], agent)
            ...
            for k in higher:
                if k != agent:
                    ...
                    upper_bound = len(self.my_map) * len(self.my_map[0])
    ...
    def find_solution(self):
        ...
        root = {
            'priority_pairs': [],
            'paths': [None for _ in range(self.num_of_agents)],
            'collisions': [],
            'cost': 0
        }
        ...
        while len(self.search_stack) > 0:
            ...
            for priority_pair in priority_pairs:
                ...
                if creates_cycle(child['priority_pairs']):
                    continue
            ...
            # TODO
            children.sort(key=lambda n: n['cost'], reverse=True)
